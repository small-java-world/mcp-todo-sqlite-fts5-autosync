...
// Try to add archived column if missing
...
`);
  ...
  upsertTask(id: string, title: string, text: string, meta: any | null, if_vclock?: number, extra?: {parent_id?:string|null, level?:number, state?:string, assignee?:string|null, due_at?:number|null}) {
    const now = Date.now();
    const row = this.getTask(id);
    if (row) {
      if (row.archived) {
        const err: any = new Error('archived'); err.code = 409; throw err;
      }
    ...
    if (if_vclock != null && if_vclock !== row.vclock) {
      ...
      const pid = (extra?.parent_id!==undefined)? extra?.parent_id : row.parent_id ?? null;
      const lvl = (extra?.level!=null)? extra?.level : row.level;
      this.db.prepare(`UPDATE tasks SET title=?, text=?, meta=?, vclock=?, updated_at=?, parent_id=?, level=?, state=?, assignee=? WHERE id=?`)
        .run(title, text, meta ? JSON.stringify(meta) : null, vclock, now, pid, lvl, st, asg, id);
      if (st !== row.state) {
      ...
      const pid = (extra?.parent_id!==undefined)? extra?.parent_id : null;
      const lvl = (extra?.level!=null)? extra?.level : 2;
      this.db.prepare(`INSERT INTO tasks(id,title,text,done,meta,vclock,updated_at,parent_id,level,state,assignee) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
        .run(id, title, text, 0, meta ? JSON.stringify(meta) : null, vclock, now, pid, lvl, st, asg);
  ...
  listRecent(limit=20) {
    return this.db.prepare(`SELECT id,title,done,updated_at,vclock FROM tasks ORDER BY updated_at DESC LIMIT ?`).all(limit);
  }
...
return p;
  ...
  const tx = this.db.transaction(() => {
    this.db.prepare(`INSERT OR REPLACE INTO archived_tasks (id,title,text,done,meta,vclock,archived_at,reason) VALUES (?,?,?,?,?,?,?,?)`)
      .run(row.id, row.title, row.text, row.done, row.meta, row.vclock, now, reason ?? null);
    // delete from FTS
  ...
  const now = Date.now();
  const tx = this.db.transaction(() => {
    this.db.prepare(`UPDATE tasks SET archived=0, updated_at=?, title=?, text=?, done=?, meta=?, vclock=? WHERE id=?`)
      .run(now, snap.title, snap.text, snap.done, snap.meta, snap.vclock, id);
    const rid = (this.db.prepare(`SELECT rowid FROM tasks WHERE id=?`).get(id) as any)?.rowid;
    if (rid != null) {
  ...
  setState(id: string, to_state: string, by?: string|null, note?: string|null) {
  const row = this.getTask(id);
  if (!row) { const e:any = new Error('not_found'); e.code=404; throw e; }
  ...
  this.db.prepare(`UPDATE tasks SET state=?, done=?, vclock=?, updated_at=? WHERE id=?`)
    .run(to_state, to_state==='DONE'?1:0, vclock, now, id);
  this.db.prepare(`INSERT INTO task_state_history(task_id, from_state, to_state, at, by, note) VALUES (?,?,?,?,?,?)`)
    .run(id, row.state, to_state, now, by ?? null, note ?? null);
  return { vclock };
  }

  addReview(task_id: string, decision: string, by: string, note?: string|null) {
  const row = this.getTask(task_id);
  if (!row) { const e:any = new Error('not_found'); e.code=404; throw e; }
  const now = Date.now();
  this.db.prepare(`INSERT INTO reviews(task_id, at, by, decision, note) VALUES (?,?,?,?,?)`)
    .run(task_id, now, by, decision, note ?? null);
  // implicit state transitions
  if (decision === 'REQUEST_CHANGES') {
    return this.setState(task_id, 'CHANGES_REQUESTED', by, note ?? undefined);
  } else if (decision === 'APPROVED') {
    return this.setState(task_id, 'APPROVED', by, note ?? undefined);
  }
  return { ok: true };
  }

  addComment(task_id: string, by: string, text: string) {
  const now = Date.now();
  this.db.prepare(`INSERT INTO review_comments(task_id, at, by, text) VALUES (?,?,?,?)`)
    .run(task_id, now, by, text);
  return { ok: true };
  }
  ...
  for (let i=0;i<lines.length;i++) {
    ...
    if (inReviews) {
      ...
      if (mr && lastTaskId) {
        ...
        this.addReview(lastTaskId, decision, by, rest ? rest.trim() : null);
        continue;
      }
      const mc = line.match(reComment);
      if (mc && lastTaskId) {
        const [__, atIso, by, text] = mc;
        this.addComment(lastTaskId, by, text);
        continue;
      }
    }
  }
